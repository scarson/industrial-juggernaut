...
- **`replayVersion`** = hash of the full **replay closure** — every file whose change re-interprets a stored log: `src/engine/** + src/rng/** + src/board/** + src/geometry/** + src/session/round.ts` (`applyEntry`) `+ src/session/hash.ts` (`stateHash`) `+ src/session/codec.ts + src/session/replay.ts`. **(Corrected 2026-06-29 during DO-host planning: the original `engine+rng+board` set was incomplete — `applyEntry` and hull/distance/`control` geometry also affect stored-log replay, so a change there MUST bump the version or in-flight games silently corrupt on the next deploy.)** **Mechanically guarded in CI** (build fails if the hash changes without a bump). Governs storage stamps, the on-mismatch path, and the promote replay-compat gate.
...
- **Timeouts — OFF by default; opt-in per room (Sam, 2026-06-29 redesign):** the defender timeout is a **room-creation toggle** in a host-layer `roomOptions.defenderTimeout = {enabled, seconds}` record (default `{enabled: false, seconds: 120}`), NOT in `RuleConfig` or `SessionRecord`. **Default (off):** a pending defender decision holds the write-lock until the human answers; liveness for a vanished player is the room creator handing that seat to an agent (Stalled acting player) — the game never auto-plays a defense for someone who stepped away, which would be game-ruining. **When enabled:** the DO arms a storage **alarm** (the only alarm consumer in v1) to `now + seconds`; the prompted human gets an **"I'm still thinking" (`extendDecision`)** control that re-arms it; only expiry *without* an extension auto-picks `representativeDefender` and appends. **Alarm idempotency** (at-least-once + retry; a bare `setAlarm(time)` carries no decision id): the handler no-ops if `pending` is absent/tombstoned (already resolved); if `pending.deadlineEpochMs` is still in the future it re-arms and no-ops (the deadline was extended); only `now >= deadline` resolves. The resolving append + the pending-clear (a tombstone) land in ONE atomic `put`, so a retry-after-failure is idempotent. Stale-`decisionId` answers get a structured "already resolved" error + resync. Hotseat/local prompts have no timeout.
...
**UI design brief:** the confirmed visual lane (warm-board / instrument-chrome blend), layout, key states, and interaction model this section builds against are specified in [the game-client UI brief](2026-06-13-game-client-ui-brief.md) (impeccable shape, brief + direction probes). The summary below stays authoritative for what the client contains; the brief is authoritative for how it looks and feels.
...
_Rulings 8–17 added from the 2026-06-13 engine-vs-rulebook fidelity audit (`docs/plans/2026-06-13-fidelity-audit-findings.md`), Sam-signed-off. The audit found **0 engine bugs**; these document existing, simulation-validated engine behavior (code is the source of truth over the stale rules doc)._

8. **Combat win-probabilities are exact bag-ratio fractions** (3→3/4, 4→5/6, 5→8/9, 6→auto), not the rulebook's rounded 83% / 89%. The engine's `combatTable` uses 5/6 and 8/9 — faithful to the velvet-bag token ratios. (Audit DER-N1.)
9. **An iron-less perimeter is legal-but-fatal, not a placement ban.** The engine permits a 4th-base placement enclosing no iron and resolves it as self-elimination (`emptyPerimeter`, no bounty) at the post-action elimination pass — matching the rulebook's "destroy *themselves* … absentmindedly" framing. (Audit DER-N2.)
10. **Factory range on tied-farthest bases:** when multiple bases tie for farthest-from-oldest, a new factory may be placed within 5 hexes of *any* of them (the symmetric resolution of a tie the rulebook leaves unspecified). (Audit DER-N3.)
11. **Attackable = convex-hull vertices** (a facet of #1): a perimetered opponent exposes only its hull vertices; a base lying on a hull *edge* (colinear between two vertices) is treated as interior and is not a legal target. (Audit DER-N4.)
12. **2-player turn order is an iron-proportional first-player draw;** when both players control 0 iron (empty bag, unspecified by the rules) the first player is chosen uniformly at random. (Audit DER-N5.)
13. **3+ player turn order:** the previous turn's last and second-to-last *surviving* players take the first two slots in random order, remaining players follow in random order; an eliminated last/second-to-last falls back to the next-latest survivor. (Audit DER-N6.)
14. **End-of-round resolution is deterministic:** (a) iron victory (≥ threshold controlled iron) precedes last-standing, tie-broken by most-iron then lowest player id; (b) elimination causes within a pass resolve noBases → broken-perimeter → noIron; (c) an all-eliminated board terminates as a victory with no winner. (Audit DER-N7.)
15. **Alliances are supported board state but have no agent-playable form/dissolve *action* in M1.** The data model is alliance-ready (`coalitions`, allied attackers, allied-aware stranding); the negotiation/consent layer lands in Phase 3 (spec §5 item 10). (Audit UNCERTAIN #1.)
16. **Board size is a tunable with oval-fit tolerance:** a size-96 request yields ~93 hexes (±6 oval fit); iron is exactly 14 via the CSP. (Audit UNCERTAIN #3.)
17. **Overlapping iron is NOT subtracted across the radiating ↔ perimeter regime boundary.** `control()` is a pure per-player function, so a radiating neighbor's 5-hex disk and a perimetered player's hull can both count the same iron hex; exclusivity is enforced only by placement bans, not by control subtraction. ⚠️ **FLAGGED (Sam, 2026-06-13) as a possible balance bug for future review** — the printed "no longer available to adjacent players that are still radiating" wording is arguably stronger than the code. Whether double-counting shared iron distorts resource races / iron-victory timing is an open question **deferred to a future balance pass** (not investigated now). If a fix is pursued, weigh it against keeping `control()` pure/cacheable (GEO-5). (Audit UNCERTAIN #4.)
