...
In `web/src/designer/NewGame.tsx`, a **memoized predicate** over the config fields that determine reachability (`boardSize`, `ironCount`, `controlRadius`, `ironHexesToWin`, `seed`, `boardSource`): generate the board via the client engine barrel, then check whether **any single first-base control disk covers ≥ `ironHexesToWin` irons** (max over `legalFirstBaseHexes` of the iron count within that hex's radius-`controlRadius` control disk). If yes → surface an advisory note through the existing note mechanism (`presets.ts` `BALANCE_IN_PROGRESS_NOTE` sibling / `NewGame.tsx:127-128` render slot): *"With these settings a single first base can win instantly — raise the board size or the iron-victory threshold, or lower the control radius."*
