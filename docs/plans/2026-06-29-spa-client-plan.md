...
**Overall:** ✅ **All 5 phases MERGED to `dev` — Deliverable 2 (the SPA client) is COMPLETE.** P4 (live play) merged in [PR #65](https://github.com/scarson/industrial-juggernaut/pull/65) (merge `45dee708`) on 2026-07-04 after Sam ran a codex-review cycle (see Discoveries P4). Two follow-ups also merged: the #44 rail-content refactor ([PR #63](https://github.com/scarson/industrial-juggernaut/pull/63)) and #46 choreography animations ([PR #66](https://github.com/scarson/industrial-juggernaut/pull/66)). P0 (PR #56 + PR #57 staging-deploy wiring), P1 (PR #58), P2 (PR #59, merge `6cb01688`), and **P3 — PR #60, merge `92003667`** — merged earlier. The game is **playable end-to-end** (real hotseat + offline-vs-agents, browser-verified; live on staging) and **playable ONLINE** over the real DO (staging E2E confirmed). Sam granted overnight merge authority for this track's PRs conditional on converged multi-round blind adversarial review (see the `spa-client-merge-authorization` user memory); #56–#60, #63, #66 merged under it; **P4 (#65) Sam merged personally after the codex-review cycle**. **The blind adversarial gate caught a CONFIRMED defect on BOTH #59 and #60** — #59: an untrusted `SessionRecord` import passed `config`/`boardSource` unvalidated into `buildFrames` (hostile paste → white-screen/hang), fixed with a shared `validateBoardSource` + config gate + call-site try/catch + seats cap; #60: `AttackComposer` re-introduced the `border` shorthand/`borderColor` longhand conflict a point-fix on `TurnOrderTokens` had left in the sibling (React re-render warning on slider-lower), fixed by sweeping the whole class (pitfall WEB-3). Both re-verified closed by a fresh blind round before merge. **P4 shipped** (P4.1–P4.5 under subagent-driven development, two-stage review per task) after a whole-branch blind adversarial security gate (NO BLOCKER — auth boundary, token discipline, reconnect/session state machine, transport-swap-held, bundle discipline all held) AND Sam's codex-review cycle (codex found the E2E-token-log [P1]; a blind follow-up caught the reload clear-on-open loop-reintroduction — both fixed; see Discoveries P4). 623 client + 2170 root green at merge.
...
| P3 — Interactive play UI + LocalReducerDriver | ✅ Merged | ~26 commits → merge `92003667` ([PR #60](https://github.com/scarson/industrial-juggernaut/pull/60)) | Routine; merged after 2 blind adversarial rounds — round 1 caught a CONFIRMED `AttackComposer` re-render defect (border shorthand/longhand, WEB-3; fixed + class-swept + re-verified closed); 507 client + 2170 root green; real hotseat browser-verified end-to-end; 3-lens phase review READY TO SHIP |
| P4 — SocketDriver / live play | ✅ Merged | [PR #65](https://github.com/scarson/industrial-juggernaut/pull/65) merged `45dee708` (2026-07-04) | **Review — socket-auth / session management**; Sam merged after a whole-branch blind adversarial gate (NO BLOCKER) + a codex-review cycle (codex [P1]: E2E logged live seat tokens; blind follow-up: the reload clear-on-open reintroduced the infinite loop — both fixed, WEB-5). Staging E2E (real Worker+WS+DO) PASS; 623 client + 2170 root green. Preceded by #44 rail-content ([PR #63](https://github.com/scarson/industrial-juggernaut/pull/63)); followed by #46 choreography ([PR #66](https://github.com/scarson/industrial-juggernaut/pull/66)) |
...
### Deviations (P1)
- **P1 (2026-07-03), six items.** (1) **`Control` type export** added to `src/index.ts` + the client barrel beyond the pre-scoped `strandedBases` (a typed `controlOf` selector needs the named interface; review adjudicated the explicit export better than `ReturnType<typeof control>` — matches the barrel's existing named-interface pattern). (2) `hexPoints` helper hoisted into `projection.ts` (the corners→points-string join lives in ONE place, per the P1.1 review note). (3) **Territory rendering is state-derived inside Board** (`territoryFills(state)` called internally, GEO-5-memoized), NOT a `BoardProps` prop — highlights stay a prop because the composer owns them; territory is game-state-owned. (4) A dev-only `/dev/board` route + test-only `web/src/board/test-fixtures.ts` (browser-verification scaffolding; P2's viewer replaces the dev page). (5) **Shape visual-weight normalization** in the P0.5 identity primitive: P1.3's browser smoke measured triangle/six-point ~50% lighter than diamond/circle at equal circumradius; per-shape scale factors now normalize filled area to the circle's (square and diamond turned out to be the same polygon rotated 45° — one shared factor), icon box widened to 1.3×size to avoid clipping. (6) `tooltipData.controlledBy` reads `territoryFills(state)`'s first (lowest-id) controller instead of hand-rolling the scan; contested-hex display copy is deferred to consumers via `overlapZones` rather than widening the tooltip shape.

### Deviations (P2)
- **P2 (2026-07-03), six items.** (1) Trio API hardening before P2.4 (quality-review-driven): `knobDescriptor(name)` single-sources control metadata (type/label/min/max/options/rows) with `validateConfig`'s bounds via one internal `KNOBS` record; generate-path board-source errors are field-keyed; `ParseBoardSourceResult` → `BoardSourceValidation`; a literal NUL byte in a test string (which made the whole file BINARY to git — diffs/blame broken) became the `\u0000` escape. (2) **`/game` temporarily renders `NewGame`** with a console-logging `onStart` — P3's game screen replaces the stub (isolated to `routes.tsx`). (3) `.claude/launch.json` (the preview-server config) is committed — team-visible browser-verification config; flag to Sam if unwanted. (4) P2.7's `EventLog` narrates CUMULATIVE events through the current frame (documented — that's what makes the 60-row tail window meaningful); reduced-motion never stops playback (frame advance is content, not decoration); the viewer uses a compact mini-form + optional `header` prop rather than embedding NewGame; imports are capped at 100k log entries pre-decode. (5) P2.8 CURATES v10 into 7 sections with all 17 DERs attached (v10-at-root stays the printed-rules source of truth; the screen teaches the digital edition). (6) `applyPreset` deep-copies independently of `defaultConfig()`'s own freshness (the module owns its copy-safety contract).

### Discoveries (P2)
- **⚠️ The bundle guard structurally never sees worker chunks:** Vite compiles ES workers as a separate Rollup build, so `.bundle-modules.json` covers only the client entry graph — the agent quarantine in `generate.worker.ts` rests on the deep-import convention plus direct grep verification of the built worker chunk (performed three times independently this phase). A future automated check would grep the built chunks; for now the convention is documented in the plugin and the pitfalls.
- **Viewer default games are ~4 frames long:** the current-playtest-config's `victoryThreshold: 10` ends all-agent games almost immediately (an engine-balance reality, not a viewer defect — the balance redesign owns it). Designers watching longer games raise the threshold; the import fixture uses 100_000.
- **jsdom lacks `ResizeObserver`** (Radix Slider constructs one on mount) — no-op stub in `web/vitest.setup.ts`; and **`userEvent` deadlocks under fake timers** (its internal delays await real time) — use `fireEvent` for timer-driven component tests. Both in `docs/pitfalls/testing-pitfalls.md`.
- **WEB-1 (CSS cascade trap):** equal-specificity utility classes compose by source order, not intent — struck TWICE in one day, both times on the screen's single brass-budget element (the Instruments button, then the designer's Start). Full entry with the compound-override + browser-computed-style discipline in `docs/pitfalls/implementation-pitfalls.md` §4.

### Deviations (P3)
- **P3 (2026-07-03), five items.** (1) **A third barrel addition beyond the two pre-scoped** (`strandedBases` P1, `isBootstrapOnly` P3.4): AttackComposer needs cube `distance` to rank fresh attackers within range (nearest-first), so the web-side engine-client barrel re-exports the existing pure `distance` from `src/geometry/cube` — **no new `src/index.ts` symbol, no `src/agent` pull** (pure math). Benign; noted because the plan pinned "expected: TWO". (2) **`keyToHex` extracted to `projection.ts`**: Hex.tsx had a private `parseHexKey`; composers parse legal-hex-set members the same way, so the inverse-of-`hexKey` parse is now one shared exported function (the GEO-2 "no pixel→hex inverse" comment preserved at the call site). (3) **`/game` shell-rail suppression** (`App.tsx` `SELF_CONTAINED_ROUTES` + a `useCurrentPath()` hook extracted from the Router): GameScreen owns its own HUD rail, so the shell's placeholder rail is suppressed on `/game` to avoid a double rail — hosting the HUD in the shell rail via a rail-content context is a deliberate **follow-up** (tracked; non-blocking). (4) **LocalReducerDriver error mapping is controller-scoped, not the honest total map**: a total `Record<WireErrorCode, DriverErrorCode>` with orphan BUILD_*/transport codes → FROZEN and `rejected.message` passing the reducer's text; the authoritative total map (which may widen `DriverErrorCode`) is scoped to **P4.1** per this plan. (5) **Inline-style `border` shorthand/longhand class fix** (three commits, `cbdf0d6d`→`ba65d3cd`; pitfall **WEB-3**): a base style using the `border` shorthand plus an emphasis variant overriding `borderColor` warns on the *de-emphasis* re-render (React removes the longhand while the shorthand remains). Struck TWICE: `TurnOrderTokens` (current-seat emphasis, caught by the live hotseat pass) and `AttackComposer` attacker spans (committed emphasis, **caught by the blind adversarial merge gate** — the sibling instance my first point-fix missed; the PR's slider test only raised commitment, never lowered it). Both fixed to longhand + raise-then-lower regression tests. `NewGame`'s `ProvenanceBadge` shares the anti-pattern but sets `borderColor` unconditionally so it never warns (latent, left as-is per YAGNI). The same sweep found the banned **side-stripe** `borderLeft` accent recurring in the P3-extracted `shell.tsx` `RuleLine` (+ `NewGame`'s note) — P2 had retired it on DER callouts but the shell extraction re-introduced it; both re-treated as a hairline frame + recessed inset (`b44d4bc4`). **Lesson:** static jsdom tests exercise mount, not re-render style reconciliation — the phase-close browser pass must *drive a state transition and read the console*; and a class fix needs a codebase-wide sweep, not a point fix (the adversarial gate proved this by catching exactly the sibling a point fix left behind).

### Discoveries (P3)
- **Choreography animations are structural-only — the reveal is unbuilt (follow-up, not a defect).** P3.9's `web/src/game/choreography/{CombatReveal,Elimination,Victory}.tsx` + `composers/TurnOrderCeremony.tsx` add `-animated` classes on the non-reduced-motion path, but there are **zero `@keyframes`/`animation:` rules anywhere in `web/src`** — no CSS defines those classes. So the set pieces render honest final content (mono numbers, shape-tagged winners) with NO actual motion; the `-animated` class is a no-op hook awaiting CSS. Content is honest and the `prefers-reduced-motion` static branch + its tests are correct — P3.9 shipped structure + reduced-motion + honest-numbers and deferred the motion design. A good `/impeccable animate` candidate (honor DESIGN.md motion energy + ease-out curves; keep the existing reduced-motion static branch as the alternative; the reveal must ENHANCE an already-visible default, never gate content visibility on the animation). Tracked as a task; non-blocking; client-only; Routine-class.
- **Static component tests miss re-render-only defects.** The `TurnOrderTokens` shorthand/longhand warning (above) passed every static render test and the 3-lens phase review's "no console.error/warn leaks" check — because that check ran the *test suite*, where no test re-rendered with a changed current seat. It surfaced only under a real turn advance in the browser. Confirms the phase-close browser pass earns its keep beyond screenshots: drive a state transition and read the console. Captured as an operational note; the regression is pinned by a re-render test.
- **AttackComposer's `eligibleAttackersFor` re-derives attacker eligibility client-side with a strict-owner filter (`b.owner === player`), not the engine's alliance-based `isAlly` filter.** The two are behaviorally equal ONLY because alliances are singletons in this phase (`Player.alliance: [id]`; `allianceOp` composition is Phase-3, per this plan's own scope boundary — "Alliances are Phase 3"). If Phase-3 alliance work grows alliances beyond singletons, this composer would silently under-populate allied attackers in the eligible-attacker candidate list (a P3-built component consumed unmodified by future alliance work, so the drift would be invisible until then). **Remediation when Phase-3 alliances land:** consume an engine `legalAttackers(state, target)` export instead of re-deriving eligibility client-side — do NOT widen the client-side filter by hand. Flagged in-code at `web/src/composers/AttackComposer.tsx`'s `eligibleAttackersFor` doc comment and in `docs/pitfalls/implementation-pitfalls.md` §4 (WEB-2). Found during the P3 composer-family refactor/polish pass (pre-P3.11), not new to this pass — the singleton-alliance assumption was already implicit in `enemyBases`'s `alliance` read; this discovery makes it explicit and durable.

### Deviations (P4)
- **P4 (2026-07-04), five items — all where the SHIPPED `src/wire`/`src/host` surface won over the plan's pre-Deliverable-1 assumptions (verified against the merged DO-host code, not the plan prose).** (1) **STALE_INDEX is a pushed `resync`, not an `error`**: on an `expectedLogIndex` mismatch the reducer replies `resync{reason:"STALE_INDEX"}` (`src/session/session.ts`), which the wire-map maps to a `sync` DriverEvent (reason dropped) — recovery is automatic; the client never "requests a resync on a STALE_INDEX rejection" as the plan's P4.3 sketch implied. The store's log-index continuity guard + the driver's logLength retrack still handle it. (2) **`hello` doubles as the initial/recovery sync**: the server answers `hello` with either `reload` (version mismatch) or a full `resync` — so the reconnect flow needs NO separate `{type:"resync"}` round-trip; the re-handshake IS the recovery request. (3) **The DO auto-answers keepalive with literal `"pong"` text frames** (`setWebSocketAutoResponse`, `game-room.ts`) — the pump ignores `"pong"` before JSON-parsing (a naive `JSON.parse` would throw). (4) **`seatClaimed` has no `DriverEvent` counterpart** — the wire-map drops it explicitly (Phase-1 issues all tokens to the creator, so nothing consumes it). (5) **`DriverErrorCode` widened to the full 33-code wire catalog** (P4.1) as an explicitly-spelled client-owned union, retiring P3's controller-scoped interim orphans→FROZEN map; 13 new honest teaching explanations landed (the typecheck-total `EXPLANATION` record forces them). The **online entry is Phase-1-scoped to a one-human-seat roster** (creator's human seat + agents; the DO drives agents server-side) — cross-device multi-human join stays Phase 2 per the plan's own boundary; a >1-human roster disables the online action with an inline note.

### Discoveries (P4)
- **The bundle guard's `!dynamicallyImported`→eager proxy false-flags a chunk shared between TWO dynamic importers (pitfall WEB-4).** Once BOTH the LocalReducerDriver and the SocketDriver dynamically imported `wire-map`, Rollup hoisted `wire-map` into a SHARED chunk that is `isDynamicEntry=false` yet is NOT statically reachable from any entry — so it loads only when a driver chunk loads (genuinely lazy), but the old proxy classified it eager and failed the build. Fixed by classifying "eager" as the forward static-import closure from entry chunks (the plan's own definition), and hardened to fail CLOSED on an entry-less (malformed) build artifact. Proven non-vacuous by a raise-then-lower (an injected eager `src/wire` import still fails the guard) AND independently by grepping the real emitted chunks (every wire/host codec symbol = 0 in the entry chunk and the separate worker chunk). This corrects a false "guard cannot detect source-injected violations" claim a mid-P4.1 probe drew from an injection that never survived into the artifact — the durable lesson (in the user-scoped learnings): in any build-guard raise-then-lower, first prove the raise changed the artifact before trusting the guard's verdict.
- **The `onopen` stale-socket identity guard needed a close-before-open superseded socket to be reachable — a re-fire-open test on an already-opened stale socket is FALSE-TEETH** (its per-socket `handshaken` flag already short-circuits before the identity guard). The reachable path: a reconnect attempt that closes before ever opening, is superseded by the next attempt, then fires a late open with `handshaken` still false. Caught when the controller's first proposed regression test would have passed even with the guard removed; the implementer corrected it and proved teeth by mutation. Lesson: an identity/guard regression test must exercise the path where the guard is actually the load-bearing check, not merely a path that reaches the guarded handler.
- **Playable online end-to-end on the real DO.** The staging E2E (`web/scripts/staging-e2e.ts`, deliberately OUT of the offline `test:client` suite) creates a room, opens a real WebSocket, completes the hello/claimSeat handshake, and plays a scripted `placeFirstBase` round-trip against the live staging Worker — the canonical no-mock test, independently confirmed PASS by the controller.

### Discoveries
- **macOS case-insensitive FS module trap (P1 close-out):** a component file whose name differs from a sibling module only by case (`Territory.tsx` vs `territory.ts`) resolves imports to the WRONG module silently — the component had to be named `TerritoryFill.tsx`. Rule: never give a `.tsx` component a case-only stem collision with a sibling `.ts` module.
- **jsdom PointerEvent gap (P1.6):** jsdom 25.x has no `PointerEvent` constructor (`TypeError`), and React 18 synthesizes `onPointerEnter`/`onPointerLeave` from bubbling `pointerover`/`pointerout` — dispatching `pointerenter` directly fires nothing, silently. Full entry in `docs/pitfalls/testing-pitfalls.md`; relevant to every P3 composer interaction test.
- **Regime-boundary witnesses need big boards (P1.4):** a size-96 board cannot host a 4-base hull whose interior clears every radius-5 disk (proven by exhaustive search) — tests that need a hull-only-controlled hex must generate size-150+.
- **GEO-5 memoization is now uniform across the board layer** (`controlOf`, `strandedHexKeys`, `territoryFills`, `highlightSets` — all WeakMap-on-state). `legalActions` is the priciest per-call derivation in the layer (full-board build-legality walk + per-target hull work), which is why `highlightSets` needed the cache most.
...
- **⚠️ `.bundle-modules.json` lands in the Worker assets dir:** the guard artifact is emitted into `dist/client/` (= `wrangler.jsonc` `assets.directory`) and contains absolute build-machine paths; only wrangler's dotfile handling keeps it unserved. When the staging deploy starts building the real SPA, exclude it explicitly (e.g. `.assetsignore`). **Related unowned follow-up:** `deploy-staging.yml` still deploys only the placeholder — NO task in this plan wires `bun run build:client` into the deploy workflow. Natural landing: end of P2, when the client is worth serving. Sam decides. **Both follow-ups addressed:** [PR #57](https://github.com/scarson/industrial-juggernaut/pull/57) wires `bun run build:client` into `deploy-staging.yml` (placeholder step kept as a no-clobber fallback) and adds `web/public/.assetsignore` (excludes `.bundle-modules.json`, confirmed via `wrangler deploy --dry-run` debug logging).
...
**Execution Status:** ✅ SHIPPED 2026-07-03 on branch `feat/web-p1-board` — all six tasks (P1.1–P1.6) plus the close-out wiring (territory fills + overlap zones rendered by Board; dev-page scenes for both regimes; hover→tooltipData readout) under subagent-driven development with two-stage review per task and review-driven hardening (GEO-5 memoization made uniform across controlOf/strandedHexKeys/territoryFills/highlightSets; shape visual-weight normalization; tooltipData DRY'd onto territoryFills). Gates at ship: 133 client tests + 2170 root green, both typechecks, build + bundle-guard green; browser-verified on the two dev scenes (radiating disks + perimeter hull with contested overlap; hover readout; zero console errors). Branch stacked on P0 was retargeted to `dev` when PR #56 merged.
...
**Execution Status:** ✅ SHIPPED 2026-07-03 on branch `feat/web-p2-screens` — all eight tasks (P2.1–P2.8) under subagent-driven development with two-stage review per task, plus review-driven API hardening before P2.4 (the `knobDescriptor` metadata API single-sourcing control types with validation bounds; field-keyed generate errors; the `BoardSourceValidation` rename; a raw-NUL-byte de-binarization of a test file) and close-out design fixes (DER callouts lost their brass side-stripe — the banned pattern; the rules subtitle moved from parchment-side ink-700 [1.48:1] to muted text-on-chrome [6.81:1, AA]). Gates at ship: 349 client + 2170 root tests green, both typechecks, build + bundle-guard green. All three screens browser-verified live, including a REAL agent game generated through the Web Worker (agents grep-confirmed in the worker chunk only, three independent verifications). 3-lens phase review (bundle-discipline-with-agents-present / brief-faithfulness / P3-readiness): READY TO SHIP, zero P2 APIs need P3 modification. Pitfalls: WEB-1 (CSS utility-composition cascade trap) + jsdom ResizeObserver + fake-timer/userEvent entries added.
...
**Execution Status:** ✅ MERGED to `dev` 2026-07-03 — PR #60, merge `92003667` (developed on `feat/web-p3-play`, off `dev` at the P2 merge `6cb01688`; branch deleted). All eleven tasks (P3.1–P3.11) under subagent-driven development with two-stage review per task, plus a composer-wave polish/shell-extraction refactor. **P3.10 executed in-phase** (UNGATED — the DO-host plan's Part A was already merged), driving the real `src/session` reducer + `src/agent` (lazy chunk) in the browser, adversarially gated. Gates at ship: **507 client + 2170 root tests green**, `typecheck` + `typecheck:client`, `build:client` + `check:bundle` green (agents confined to the `local-reducer-driver-*.js` lazy chunk — entry + eager route chunks carry zero `src/agent`; the guard is now LIVE via GameScreen's dynamic import). 3-lens phase review (authoritative-state / a11y / bundle discipline): READY TO SHIP, clean pass on all three against source. **Real hotseat browser-verified end-to-end** (human base placement → greedy agent auto-place → setup→play with contextual Build/Attack composers → optimistic-preview budget decrement → build commit narrated in the event log + factory gauge increment → agent-drive advancing the agent seat → both territory regimes rendered). The live interactive pass caught one re-render defect the static suite missed (`TurnOrderTokens` border shorthand/longhand mix warned on turn advance — fixed `cbdf0d6d`, regression-tested). See Deviations (P3) + Discoveries (P3).
...
### Task P3.10: LocalReducerDriver — ✅ SHIPPED (Part A was live)

**Execution Status:** ✅ SHIPPED — executed in-phase against the real, already-merged `src/session` reducer. The shipped surface was read first (per the R4 note): `submit` runs `applyCommand` then the reducer's own agent-drive loop (`needsDrive`/`driveOneStep`/`agentForSeat`), and derives pending eligibility via the reducer's `eligibleDefenders`/`toWirePending` — no round-state reimplementation. `Effects` translate to `DriverEvent`s (`broadcast`→`applied`/`turnRollover`/`gameOver`; `toSeat`→`prompt`; `reply` error→`rejected`), decoding broadcast-carried encoded entries; `persist`/`alarm` ignored (no storage/defender-timeout locally). WireErrorCode→DriverErrorCode is a total `Record` (orphan BUILD_*/transport codes → FROZEN; `rejected.message` passes the reducer's text). Loaded via GameScreen's dynamic `import()` → agents land in a separate 24.66 kB lazy chunk. Adversarially gated + a real hotseat browser pass. **Error-mapping note:** the honest total map is scoped to P4.1 (which may widen `DriverErrorCode`); this phase's mapping is the controller-scoped decision (see Deviations).
...
- [x] **Browser verification:** the **real LocalReducerDriver** path (not just the fake) was exercised live — setup placement, agent auto-place, setup→play transition, contextual Build/Attack composers, optimistic-preview budget decrement, build commit narrated in the event log, factory-gauge increment, agent-drive advancing the agent seat, both territory regimes rendered, console error-clean after the `TurnOrderTokens` fix. The harder-to-reach states (defender prompt, combat reveal, elimination, victory, chain-continue, forced-pass, ceremony) are covered by the P3.11 integration structure tests + each component's own suite (reduced-motion branch tested per choreography component with mocked matchMedia).
- [x] **LocalReducerDriver gate:** Part A had shipped, so P3.10 executed in-phase and a **real end-to-end hotseat ran in the browser** (human-vs-greedy). Not deferred.
- [x] **Phase review (≥3 perspectives):** (1) authoritative-state rule held (fold-`applyEntry` + replace-on-`sync`; preview never authoritative, never combat — verified: every store event case clears preview, the `attack` preview returns state unchanged + `{combat:true}`, the driver never reimplements round state); (2) a11y — every legal action is a real focusable `<button>` via the shared `HexButtonList` (G4), prompts carry `role="note"`/`group`/`list`, GameScreen's composer switch traps no focus, reduced-motion branch present + tested in all four choreography/ceremony components; (3) bundle discipline — agents ONLY in the `local-reducer-driver-*.js` lazy chunk, the guard genuinely guards (a static import would fold 6 agent modules into the eager entry chunk → `assertNoAgentsInEager` fires), fake-driver confirmed absent from every built chunk. Result: **READY TO SHIP**, clean pass on all three. Banner + table updated; Deviations/Discoveries recorded below.

---

# PHASE P4 — SocketDriver / live play

**Execution Status:** ✅ SHIPPED at `85a7a3fb` on 2026-07-04 ([PR #65](https://github.com/scarson/industrial-juggernaut/pull/65) merged at `45dee708`). All five tasks (P4.1–P4.5) executed under subagent-driven development with two-stage (spec + quality) review per task, plus the close-out: an independently-confirmed staging E2E (real Worker + WebSocket + DO round-trip), a browser pass driving state transitions (online-entry gating both directions, local play with the HUD hosted in the shell rail, pristine console), and a whole-branch blind adversarial security gate — **NO BLOCKER** across the agent-seat auth boundary, token discipline, malformed-response handling, the reconnect/session state machine, transport-swap-held, and bundle discipline (independently grep-verified: zero wire/host codec symbols in the entry chunk AND the separate worker chunk). Mid-flight review caught + fixed a P4.2 `requestSync` readyState blocker, a P4.3 backoff-reset crash-loop, a false-teeth `onopen`-guard test, and (post-gate, non-blocking) a createRoom untrusted-response defense-in-depth gap. **Sam then ran a codex-review cycle before merge:** codex found a [P1] (the staging E2E logged live seat tokens); a blind Opus follow-up caught that the reload-guard clear-on-open fix reintroduced the infinite reload loop (`connection:"open"` precedes the server's reload verdict — reverted to fail-safe never-clear; pitfall WEB-5) — both fixed + closure-verified, then merged. Gates at merge: 623 client + 2170 root green, both typechecks, `build:client` + `check:bundle` green. The shipped `src/wire`/`src/host` surface was read first and won over the plan's pre-Deliverable-1 assumptions on four points (STALE_INDEX-as-pushed-resync, hello-doubles-as-sync, `"pong"` auto-answer frames, `seatClaimed`-dropped — see Deviations (P4)).

---

**Original task spec (retained for reference):** ▶ READY — the gate is OPEN: the DO-host plan's **Part B** (the `GameRoom` Durable Object + Worker + a staging-validated deploy) AND **`src/wire`** (Part A1) have shipped (all of Deliverable 1 is merged; confirm the exact surface by reading `docs/plans/2026-06-29-do-host-wire-protocol-plan.md`'s Part B banner + the shipped `src/wire`/`src/host` before writing the SocketDriver). It is security-sensitive (socket auth / session management). At execution: honor the resolved agent-seat auth boundary (DO-host plan's B6.2/B2.2 + the reducer envelope backstop — a socket must never bind an agent seat), and author the honest total `WireErrorCode → DriverErrorCode` map in P4.1 (may widen `DriverErrorCode` — the P3 LocalReducerDriver used a controller-scoped interim map).
