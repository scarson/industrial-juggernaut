...
**Overall: ✅ PLAN COMPLETE — Definition of Done met 2026-07-03.** PART A FULLY MERGED to dev 2026-07-02 (PRs #35, #36, #37, #40, #43, #42). PART B FULLY MERGED to dev 2026-07-02/03 (PRs #44, #45, #46, #48, #49, #50, #51, #52, #53). The staging deploy fired on both post-merge `dev` pushes (Actions runs 28640229563 + 28640409556, both success); the Worker is live at **https://industrial-juggernaut-staging.samuel-carson.workers.dev** (verified: HTTP 200, placeholder page). Handoff: `docs/plans/2026-07-03-deployable-client-track-handoff.md`.
...
| B1 — Shared-config scaffolding | ✅ Merged | 9dd81593…c95ad8cb → PR #44 | vitest 4.1.9 across the suite; committed Cloudflare.Env (no gitignored generated .d.ts); replay-version guard |
| B2 — Worker shell + room addressing | ✅ Merged | c1f2de5f, 7ac0e334, a1de3906 → PR #45 | first deployable Worker; HUMAN-ONLY token minting; adversarial hardening (5 exploit classes) |
| B3 — GameRoom DO: storage + critical section + recovery | ✅ Merged | ccd9d54c…cf12da9f (+CI-env fix 749abbab) → PR #46 | flagship op-order test; freeze-path throw variants fixed |
| B4 — Hibernation | ✅ Merged | 01ba6709 (+SECURITY marker) → PR #48 | ping/pong auto-response; per-command sink; empty-seat bind bug fixed |
| B5 — Defender-timeout alarm (opt-in) | ✅ Merged | 3d2dff32 (+liveness comment) → PR #49 | idempotent alarm(); chainAttacker replication byte-equiv to command path |
| B6 — Socket attribution + malformed-traffic enforcement | ✅ Merged | 127b2ebc, 960f2da8, fc5ee54b → PR #50 (Review, Sam merged) | adversarial gate caught + fixed a CONFIRMED DoS (WIRE-SHAPE-1); auth layer-2 |
| B7 — vitest-pool-workers DO test suite | ✅ Merged | 55378eee → PR #51 | 8-scenario integration matrix; green-first (no cross-component bug) |
| B8 — deploy-staging.yml + version guard + CI wiring | ✅ Merged | 1307eaeb, 71a95359 → merge 311ca184 | PR #52 (Review, Sam merged 2026-07-03); CF secrets set as Repository secrets (correct for job-level refs); no CI split (bun runs the pool); staging deploy fired on the merge push and succeeded |
| B9 — DO/wire pitfalls documentation | ✅ Merged | 91cbf597, 2b387059 → merge f579d68f | PR #53 (Routine, agent-merged after #52); 11 DO-*/WIRE-* entries + the Deliverable-1 handoff |
...
- **⚠️ B6.1 obligation (B5 review, 2026-07-03): `trySend` must make `sendEffects` failure-tolerant** — a throw inside a send (a dead/closing socket) BETWEEN the awaited persist and the trailing `driveAgents()` strands a rolled-to agent turn until the next wake (self-heals via `rehydrate`'s tail-drive on a cold retry, but a warm-instance retry no-ops at the pending-tombstone guard). Present in BOTH `alarm()` and `handleCommand`. B6.1's `trySend` (wrap `ws.send` in try/catch, mark the socket gone on failure) closes the window for both. Documented inline at both call sites. **Update (PR #54, `19fdf836` on dev, from background `task_8a3ae690`):** the inline note was rewritten present-tense — because `trySend` routes every send non-throwing, `sendEffects` *cannot* strand the drive at all; the sole residual throw between the awaited persist and the drive is a **storage error in `realizeAlarm`** (same self-heal/warm-retry behavior). Net: the send-throw path is fully closed; only a persist-layer alarm-write failure remains. See handoff Seam 4.
...
**Execution Status:** ✅ MERGED — PR #52 (Sam, 2026-07-03, merge 311ca184). Shipped 2026-07-03 on branch `feat/host-ci-deploy` (stacked on the B7 PR #51 branch). Replay-version guard added to the required `check` job; `deploy-staging.yml` created. CI-log evidence (dev run 28633567530) confirms the workerd host pool runs under bun in `check`, so the plan's Node-split is unnecessary — see Deviations. **Staging deploy VERIFIED:** the workflow fired on both post-merge `dev` pushes (Actions runs 28640229563 for the #52 merge, 28640409556 for the #53 merge; both success) and the Worker is live at **https://industrial-juggernaut-staging.samuel-carson.workers.dev** (HTTP 200, placeholder `index.html` from `scripts/ensure-placeholder-assets.ts` — the SPA build replaces it in deliverable 2).
...
**Execution Status:** ✅ SHIPPED 2026-07-03 on branch `feat/host-pitfalls` (stacked on the B8 PR #52 branch). Added `docs/pitfalls/implementation-pitfalls.md` §Section 2 (Durable Object Host: DO-PURITY-1, DO-CODEC-1, DO-ORDER-1, DO-STORAGE-1, DO-ID-1, DO-AUTH-1, DO-ALARM-1, DO-HIBER-1, DO-TEST-1) and §Section 3 (Wire Protocol: WIRE-MAP-1, WIRE-SHAPE-1) — 11 entries, each with full Flaw/Why It Matters/Fix/Lesson, verified against the shipped code (not the plan's speculative wording where the two differ — e.g. DO-STORAGE-1's pending-clear rides as an in-put tombstone, never a `transaction`, matching DO-ORDER-1/the shipped `persistEvent`). Both session Discoveries folded in: WIRE-SHAPE-1 (wire-boundary full-shape validation, confirmed DoS at B6) and WIRE-MAP-1 (engine throw messages load-bearing for the session error mappers, A3.3). Maintenance-framework checklist complete: TOC updated (3 new rows), each section's Review Checklist added, Appendix B summary table has one row per entry, Appendix A changelog dated 2026-07-03, cross-ref DO-ID-1 ↔ GEO-3 added bidirectionally. `bun run typecheck` green (docs-only; confirms no drift). Shipped as commit 3964ba64.
...
- [x] **Step 1:** add the entries + complete the maintenance-framework checklist (IDs, review checklist §, TOC, Appendix B table, cross-refs).
- [x] **Step 2:** verify the doc renders + the completeness checklist is done; commit `docs(pitfalls): add DO-* + WIRE-* entries (purity/codec/ordering/storage/id/auth/alarm/hibernation/test/mapping/shape)`.
- [x] **Apply the Execution Discipline block** (docs scope — no TDD; the gate is the completeness checklist).
