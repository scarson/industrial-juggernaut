...
**Overall:** PART A FULLY MERGED to dev 2026-07-02 (PRs #35, #36, #37, #40, #43, #42). **PART B: B1–B7 MERGED to dev 2026-07-02/03** (PRs #44, #45, #46, #48, #49, #50, #51). **B8 (PR #52, Review — Sam merges + set CF secrets) and B9 (PR #53, Routine — agent-merges after #52) are the last two open, rebased conflict-free onto dev.** Once both land + the first `dev` push deploys staging, the plan's Definition of Done is met. Handoff: `docs/plans/2026-07-03-deployable-client-track-handoff.md`.
...
| B8 — deploy-staging.yml + version guard + CI wiring | ✅ Shipped, PR #52 OPEN | 1307eaeb, 71a95359 (rebased) | **Review — SAM MERGES**; CF secrets set as Repository secrets (correct for job-level refs); no CI split (bun runs the pool); staging deploy fires on first dev push after merge |
| B9 — DO/wire pitfalls documentation | ✅ Shipped, PR #53 OPEN | 91cbf597, 2b387059 (rebased) | Routine — agent-merges after #52; 11 DO-*/WIRE-* entries + this handoff |
