...
### Discoveries
- **B6 boundary question (A5.1 quality review, 2026-07-02): the reducer never checks `config.kind` on the acting seat** — a socket authenticated with an AGENT seat's token could send mutating commands for that seat (the envelope checks turn ownership, not seat kind). Defused in Phase 1 (all seat tokens are minted to the trusted room creator, B2.2), harmless at the claimSeat layer (ephemeral state only). **B6/B2 executor should decide whether the WS upgrade or the DO should refuse to bind sockets to agent seats** — must be closed before Phase 2 cross-device join.
- **⚠️ B3 MUST handle the no-gameOver-broadcast case (A4.5 quality review, 2026-07-02):** a mid-setup victory produces NO `gameOver` broadcast — `placeFirstBase` entries never close a round, so `commitEntries` never emits one; the drive loop just stops (`needsDrive` false). Live path (probed: 4p default-config, 3/5 seeds). The DO host (B3.2/B3.3), after its driveAgents loop exits, MUST check `status(session.game)` and broadcast `gameOver` itself when terminal-without-emission (or an equivalent mechanism) — otherwise clients in a mid-setup-decided room never learn the game ended.
...
- [ ] **Step 1-5:** failing test (ids are the right length/charset; digests are stable + 64 hex chars; two `newRoomId` calls differ) → implement → commit `feat(host): crypto-random room ids + seat tokens + digest`.
- [ ] **Apply the Execution Discipline block.** (Tests live in `test/host/**` → workers pool.)
...
- [ ] **Step 2-5:** implement; commit `feat(host): Worker fetch — room create + ws upgrade routing`.
- [ ] **Apply the Execution Discipline block.** Review-class.
