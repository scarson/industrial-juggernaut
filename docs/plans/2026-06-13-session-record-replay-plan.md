...
- **Spec coverage (§3 + §7 record/replay slice):** `SessionRecord` shape → 1.1; codec → 1.2; `stateHash` → 2.1; `LogEntry` union + the per-kind replay/round state machine (rngBeforeApply install, per-declaration composition, advanceRound driving, status-once-per-round) → 3.1; setup-phase `placeFirstBase` logging for all seats + agent auto-pick → 4.1; the canonical per-declaration composition (NOT `stepRound`/`applyAttack`) → 3.1/4.1; replay equivalence property tests (terminal state, advanceRound-count, per-boundary stateHash) → 5.1; mid-turn elimination + bounty/stranding timing + regime boundaries → 5.2; bigint codec >2^53 → 1.2; defense-in-depth validation (§3 checks 1/3/4a/4b/5; check 2 single-decl deferred to plan 2's parser) → 6.1; the record↔replay anti-tautology cross-checks against the live driver → 4.1; session barrel surface → 6.2. **Deliberately deferred to plan 2/3/4** (noted in Discoveries): interactive `GameSession` reducer (human commands, `expectedLogIndex`, pending/defender substitution, seat-claim, resync, wire events), the DO host (storage/recovery/hibernation/alarm/Worker/wrangler/vitest-pool-workers/deploy-staging/replayVersion guard), and the §6 production cutover.
...
- **Anti-tautology cross-checks (codex round 2):** because record AND replay both route through `applyEntry`, Task 4.1 adds two cross-checks against the BATTLE-TESTED live driver — (1) `recordGame` outcome == `runGame` outcome (same seed), and (2) `recordGame.finalState` deep-equals a `stepRound`-driven reference mirroring `src/driver/run.ts`. A wrong `applyEntry`/`rngBeforeApply` that fooled the record↔replay tests gets caught here. Heuristic seats (variable policy draws) get their own replay test.
- **Validation completeness (codex round 2):** §3 check 4 is two facets — `validateTargetAttackable` (no-eligible-defender → unattackable) AND `validateAttackDecl`'s submitted-defender re-validation (fresh/range/owner/≠target, mirroring the engine). Check 2 (single-decl) is deferred to plan 2's command parser, named for traceability.
- **Engine-barrel purity preserved (round 1):** the agent-dragging session surface lands on a SEPARATE `src/session/index.ts`; `src/index.ts` stays agent-free (foundation Phase 6's property).

## Plan review cycle record (`plan-review-cycle`)

4 rounds (min 3), alternating runner self-review with an independent cross-model reviewer; ran until two consecutive rounds landed 0 substantive findings.

- **Round 1 — runner (Claude/Opus):** 5 findings, all fixed. (A) **Barrel purity** — Task 6.2 put `recordGame` (which imports `src/agent`) on the agent-free engine barrel `src/index.ts`, breaking foundation Phase 6's purity property → split into a separate `src/session/index.ts`. (B) `compose` used an ugly `GameState["players"][number]["id"]` → `PlayerId`. (C) validation naming/signature drift (`validateAttackDecl` vs a referenced `validateAttackEligibility`). (D) property-test runtime "floor vs tune-down" tension reconciled. (E) Task 5.2 missing a "do NOT add engine code to force a regime" boundary.
- **Round 2 — codex (OpenAI, cross-model, read-only, `high` reasoning, ~116k tokens):** ~9 findings (2 P1 + 1 P1-ish + ~6 P2), all applied. **P1: tautological tests** — record AND replay both route through `applyEntry`, so a wrong `applyEntry` passes record↔replay while diverging from live semantics → added two cross-checks against the battle-tested `runGame`/`stepRound` (Task 4.1). **P1: `RuleConfig` import** — imported from `../engine/types` which doesn't re-export it (only `RngState`) → fixed to `../engine/config` (would have failed typecheck). **P1-ish: submitted-defender re-validation** missing (§3 check 4b) → added `validateTargetAttackable` (4a) + `DEFENDER_INELIGIBLE` in `validateAttackDecl` (4b). P2s: terminal-handling contradiction (Phase 3 vs 5) resolved; `RecordResult.events` added (Task 5.2 needed them); heuristic-seat replay test added (the variable-draw RNG path); seed-codec exception documented; check-2 framing corrected; `entry.player===currentPlayer` assumption documented. **Codex independently verified the `rngBeforeApply` capture model and the per-kind composition are correct against the real `greedy`/`heuristic`/`stepRound` source** — the highest-value confirmation.
- **Round 3 — runner (Claude/Opus):** 0 substantive (2 cosmetic): a `.ts` import extension inconsistent with the codebase; a stale self-review section. Fixed.
- **Round 4 — runner (Claude/Opus):** 0 substantive (1 cosmetic): grep-based consistency scan confirmed no stale references, all symbols/codes/exports consistent, barrel correctly targets `src/session/index.ts`; fixed one stale "validation 1–5" self-review phrase. Cycle complete.

The independent cross-provider round (Round 2, codex) is again where the highest-impact findings came from — the tautological-test gap and the `RuleConfig` typecheck bug were both author blind spots my self-review missed. That is the alternation mechanism working as intended (consistent with the project's cross-model-plan-review learning).
