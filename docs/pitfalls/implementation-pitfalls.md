...
### WEB-4: A Module Imported by Two Dynamic Chunks Becomes a Shared Chunk the `!dynamicallyImported` Bundle-Guard Proxy False-Flags as Eager

**The Flaw:** The bundle guard (`web/scripts/check-bundle.ts`) must prove `src/agent` + `src/wire` never ship in the eager entry payload. Its v1 classification used the proxy `isEntry || !dynamicallyImported → eager`, relying on the assumption that a lazy-only module always lands in a chunk with `isDynamicEntry === true`. That holds only while each lazy module has exactly ONE dynamic importer. The moment a second dynamically-imported chunk statically imports the same module, Rollup hoists it into a **shared chunk** that is imported (statically) by both dynamic chunks — and that shared chunk has `isDynamicEntry === false` (it is not itself an `import()` target, just a common dependency). The proxy then classifies the genuinely-lazy shared chunk as eager and fails the build, even though nothing in the entry's static-import closure reaches it.

**Why It Matters:** P4.5 wired the SocketDriver as a second dynamic importer of `web/src/game/wire-map.ts` (the first is the LocalReducerDriver — both loaded via `import()`). Rollup produced a `wire-map-*.js` shared chunk carrying `src/wire/codec` + `src/wire/protocol`, `isDynamicEntry: false`. `check:bundle` went red claiming `src/wire` had leaked into an eager chunk — but it had NOT: the built entry chunk (`index-*.js`) statically imports nothing (`imports: []`), reaches both drivers only through `import()`, and the shared `wire-map` chunk loads only when a driver chunk loads. The guard's own comment even asserted the false premise ("a statically reachable module always stays in a static chunk … so this flag safely means lazy-only"). A naive "fix" — collapsing both drivers behind one dynamic barrel to kill the shared chunk — is architecturally wrong: it would put `src/agent` (the reducer's agents) and `src/wire` (the socket codecs) in the SAME chunk, so a socket player downloads the entire agent AI and a local player downloads socket code, defeating the whole point of the split.

**The Fix:** Classify eager by **forward static-import reachability from entry chunks**, not by the `!dynamicallyImported` proxy. Record each chunk's static import edges (Rollup `chunk.imports`) in the emitted `.bundle-modules.json` (add `staticImports` alongside `moduleIds`). Compute the eager set as the transitive closure from every `isEntry` chunk following ONLY `staticImports` (dynamic edges are lazy boundaries — never followed), visited-guarded against static-import cycles. A shared-of-two-dynamic-chunks module is not forward-reachable from the entry and is correctly lazy; a chunk the entry statically pulls is correctly eager. Keep the guard non-vacuous: verify it still throws when a lazy-only module is injected into the entry's static closure (it does). This is the classification the plan named all along ("eager = chunks reachable from an `isEntry` chunk via static imports"); the proxy was a documented v1 shortcut.

**The Lesson:** A reachability property (is this module in the initial payload?) must be checked by actual reachability over the right edge type, not a local per-chunk boolean that happens to correlate under a narrow assumption. `isDynamicEntry` answers "is this chunk an `import()` target?", which is NOT the same question as "is this chunk lazy?" — a shared dependency of two lazy chunks is lazy but is no one's dynamic entry. The proxy held for one dynamic importer and broke silently at two; the correct guard walks the static-import graph from entries. When a fail-closed gate goes red, confirm whether the property it protects is actually violated (grep the built entry chunk; inspect the module map by source path) before touching either the gate or the code — the gate can be wrong in the safe direction, and "make the red go away" can mean loosening a real protection OR breaking a good architecture.
