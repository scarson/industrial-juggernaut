...
- [ ] **jsdom has no `ResizeObserver`, and Radix UI primitives construct one on mount.** Rendering a Radix component that measures itself (Slider, at minimum) in a jsdom test throws `ReferenceError: ResizeObserver is not defined` the instant it mounts — jsdom implements no layout engine, so it never shipped one. The fix is a no-op stub registered once, globally, before any test file runs: `web/vitest.setup.ts` installs a `ResizeObserverStub` (constructor + no-op `observe`/`unobserve`/`disconnect`) on `globalThis.ResizeObserver` if the global is absent. Any test that mounts a new Radix primitive for the first time relies on this stub already being in place — if a future primitive needs something the stub doesn't provide (a real observed size, an actual resize callback firing), that's a signal the check belongs in real-browser verification (`preview_inspect`/`preview_screenshot`), not jsdom; the stub only exists to let mount-time construction succeed, not to simulate layout.
- [ ] **`vi.useFakeTimers()` + `@testing-library/user-event` deadlocks; use `fireEvent` under fake timers.** `userEvent` schedules its own internal timers (per-keystroke/click delays) that never fire while the clock is fake unless you both pass `userEvent.setup({ advanceTimers: vi.advanceTimersByTime })` AND keep advancing — in practice an `async` `user.click`/`user.paste` in a fake-timer test just hangs to the 5000ms test timeout. For any test that needs fake timers (timer-driven playback, debounce, polling), drive the DOM with synchronous `fireEvent` (`fireEvent.change` to set an input value, `fireEvent.click`) instead of `userEvent`, and load component state through a **synchronous** path so there's no promise to flush under the frozen clock. **🔥 Found writing `web/src/viewer/AgentViewer.test.tsx` (P2.7):** the play/pause tests originally generated frames via an `await`ed injected `generateGame` then `userEvent.click`'d Play — both the awaited promise and userEvent's timers stalled under fake timers (two 5s timeouts). Fix: load frames via the synchronous import path (`parseSessionRecord` runs in the click handler, no await) and use `fireEvent` throughout — see that file's `importFixture()` helper and the `play / pause on a timer` describe block.
- [ ] **Reading DOM state in the SAME synchronous `preview_eval` that dispatched the click reads the PRE-commit value.** React commits state updates asynchronously; a browser-smoke probe that clicks a button and reads `el.textContent` in one `eval` sees the old value and looks like a no-op bug. Split the click and the read into two separate `preview_eval` calls (React commits between them), or the smoke test will chase a phantom regression. **🔥 Found in the P2.7 AgentViewer browser smoke:** "Step forward doesn't advance" was a false alarm — the step handler fired correctly; the immediate same-tick re-read just showed the stale frame position. A second `eval` reading `[data-testid="frame-position"]` confirmed it had advanced.
...
- [ ] **Synthetic boards routed through `commitEntries`/`applyEntry` give every surviving player controlled iron.** An empty-iron synthetic `GameState` (e.g. `board.iron: []`) is fine for a pure-function probe, but the moment a test applies a REAL action via `commitEntries`/`applyEntry` (`src/session/agent-drive.ts` / `src/session/round.ts`), `applyEliminations` runs after every entry and unconditionally eliminates (`noIron`, `src/engine/status.ts`) any player with ≥1 base but zero controlled iron; give every surviving player iron ON one of their own base hexes (controlled in both radiating and perimeter mode — see GEO-8, `docs/pitfalls/implementation-pitfalls.md`) or a full 4-base non-degenerate perimeter of their own (immune to GEO-8's non-ally-perimeter exclusion) — iron at an arbitrary nearby hex is not safe, since a mid-test base-count change can flip which control mode governs that hex. **🔥 Found in `test/session/auto-close.test.ts` (Task A4.2, commit `e52039af`):** a real attack applied via `commitEntries` against an empty-iron synthetic board (the `pending.test.ts` `synthGame` pattern, safe there only because those tests never leave pure-function probes) wiped out both synthetic players mid-test; after adding on-hex iron, a captured base crossing the 4-base perimeter threshold then flipped the winning attacker into perimeter mode, enclosing the other synthetic player's iron inside its hull and (per GEO-8) eliminating that still-radiating player too — see the `describe` blocks `autoCloseIfNoAttack — attacker exhausted` and `autoCloseIfNoAttack — legal attack remains` for the worked fixtures.
