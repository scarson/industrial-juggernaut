...
- **Refuted hypothesis — "label substitution":** last-standing wins were not, in aggregate, near-miss iron strategies relabeled by the per-action elimination check: nobody reached 12, 11 of the 13 winners peaked at ≤9, and the closest players (g16's winner at 11/12, g5's at 10/12, g14's victim at 11/12) were STALLED at that count for multiple turns rather than converging. g16 is the one genuine near-miss-relabel candidate in the sample.
...
- **Iteration dose-response (CRN-paired, same boards):** at **iters=100 AND iters=300**, both probed clock-regime games were **move-for-move identical** to iters=30 — every action, elimination, turn, and iron value unchanged across the full 30→100→300 ladder (data: `data/probe-b150sparse-i{30,100,300}.jsonl`). The agent advances game RNG by exactly one step per move regardless of budget, so identical trajectories mean the most-visited root action never flipped at any of the 50 player decisions across the two games (52 snapshot rows counting setup) at any rung. iters=300 is production sweep-agent strength (`defaultMctsParams()` uses 300; `defaultMctsCoreParams()`'s 1000 is not used by any gameplay path). **The factory-clock regime is iteration-budget-robust on these boards (n=2, directional);** the full Phase-1 ladder extends n and adds the maxDepth rung.
...
- `probe-b150sparse-i30.jsonl` / `probe-b150sparse-i100.jsonl` / `probe-b150sparse-i300.jsonl` — sparse board-150 regime + full 30→100→300 dose-response ladder.
...
**Evidence-tier caveat (session-scratch figures, not backed by committed data):** the ~8.8% dead-outer-ring-hex rate and the 0/640 default-board scoping, the weak-agent 6–29-base perimeter range, the samplePolicy 200/200 retraction probes, and the ~5.5 min/game timing were measured in the design session's scratch environment and are reproducible from the described methods (deterministic seeds) but their raw outputs are not committed. The Phase-1 ladder and instrumentation re-derive the load-bearing ones on the committed path; treat the rest as reported-method figures. On the clock-death attribution: the 5/34 "only-possible-live" count is exact; the "~10 necessary / ~20 entangled" figures are methodology-dependent estimates whose categories overlap with the 9/34 self-built count — the three-way instrument in Phase 1b replaces them with measured values.
