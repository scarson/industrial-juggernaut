...
| Test suite re-run | Tests haven't been run end-to-end since the MCTS changes (preserveSoftmaxPrior + rootBootstrap). Type-check is clean, and the eight test files I saw passing during a partial vitest run before killing it were all green, but full suite was never re-run. | Just `npx vitest run` |
