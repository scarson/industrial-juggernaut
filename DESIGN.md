...
### Hierarchy
The per-surface rem scale is realized inline on the shipped screens (there is no central type-scale token file yet); product-register defaults govern it: fixed rem values (no fluid clamp headings), a tight ratio (1.125–1.2), prose capped at 65–75ch, data rows running denser — mono figures around 0.8–0.85rem, the factory gauge stepped up to 1.1rem. Weights in use: 400 (working text) and 600 (emphasis, display moments) — the two static cuts each self-hosted face ships.
...
### Navigation
- **Top bar** (`.shell-topbar`): slim, ≤44px (`--layout-topbar-height`), on the table surface: wordmark (Cartouche serif — the title plate, doubling as the home link), turn/phase chip, seed/config readout in mono, and the Instruments button (brass). The chip, readout, and Instruments button are instruments — each renders only when it has a job (a game supplying labels; a wired menu), never as a dead placeholder: brass never sits on an inactive control.
- **Right rail:** ONE collapsible complementary landmark (`aria-label="Rail"`), a fixed-width instrument column (19rem — content never sizes it; the board always wins space); expanded at the wide tier (≥1100px), collapsed behind a `.chrome-button` toggle with `aria-expanded`/`aria-controls` (always-mounted, `hidden`-toggled panel) below it. The shell owns the rail and hosts the game HUD into it via a rail-content context, and the rail mounts ONLY while a screen has published instruments into it — a rail with nothing to hold earns no pixels, so the landing, viewer, and rules screens get the full table — the rail is not a per-screen widget. Routing is an in-house pushState router (4 static routes) — no router dependency.
...
The action surface beside the board is ONE composer at a time, resolved from game state (`web/src/composers/`): build, attack, defender prompt, chain-continue, forced-pass, setup placement, and the turn-order ceremony, sharing an extracted `ComposerPanel`/`RuleLine` shell. They wear the working type and the mono face for numbers — never the display serif (the Cartouche Rule); a composer is instrument-panel, not a game moment. The a11y path is each composer's own list of real, keyboard-reachable hex buttons; the SVG board carries the same actions for pointer play (clicks route to the mounted composer through the store's board-handler channels, and only highlighted legal cells afford a click), so the board is an action surface whose keyboard equivalent is always the composer's button list. Combat odds are shown before the draw in mono — honest tension — and rejected commands teach the rule they broke (`explainError` over the full wire error catalog) rather than surfacing a bare code.

### HUD (right-rail instruments)
The rail's game content: per-player resources (iron / factories / bases per seat), the shared factory-supply gauge (a `placed / total` mono readout — 36 by default, but the `factorySupply` knob is tunable, never a hard-coded literal), the turn-order tokens (current seat carrying the brass emphasis), and a virtualized event log narrating `GameEvent`s (windowed to the tail, reused from the viewer). Elimination and bounty are not a HUD counter — they surface in the event-log narration and, at the moment they happen, in the Elimination set piece. It is published INTO the shell rail via the rail-content context, not rendered as its own landmark — one rail, hosting whatever the current screen provides.
